...
def load(tag):
    rows = list(csv.DictReader(open(os.path.join(DATA, f"scope_{tag}.csv"))))
    out = {}
    for p in POLS:
...
def main():
    ...
    fig, axes = plt.subplots(2, 5, figsize=(12.5, 4.6))
    for j, (tag, xlab, xscale) in enumerate(PANELS):
        ...
        axes[0, j].set_title("(%s)" % "abcde"[j], loc="left", fontsize=9)
    ...
    out = os.path.join(HERE, "..", "figures", "scope_sensitivity")
