#!/usr/bin/env python3
"""ISL-availability sweep at U_TGT=0.15 (mirrors src/14 isl_sweep)."""
import os, sys, csv
import numpy as np
...
if __name__ == "__main__":
    ...
    with open(os.path.join(OUT, "tleP15_isl.csv"), "w", newline="") as f:
        w = csv.writer(f)
        w.writerow(["p_isl", "split_mean", "split_std", "minfill_mean", "minfill_std",
                    "totQ_mean", "down_mean"])
        w.writerows(rows)
    print("wrote tleP15_isl.csv", flush=True)
