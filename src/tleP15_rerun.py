#!/usr/bin/env python3
"""Re-run the TLE primary batch at the new feasible mainline target U_TGT=0.15.

Same protocol as src/14_tle_primary.py (TLE trace, primary config), but with
M.U_TGT = 0.15.  Writes data/tleP15_{core,stability,panel,battery,pbase,arrival}.csv
...
def save(tag, rows, hdr):
    with open(os.path.join(OUT, f"tleP15_{tag}.csv"), "w", newline="") as f:
        w = csv.writer(f); w.writeheader if False else None
        w.writerow(hdr); w.writerows(rows)
    print(f"wrote tleP15_{tag}.csv", flush=True)
...
def stability():
    ...
    b0 = r["log"]["battery"][:, 0]
    np.savez_compressed(os.path.join(OUT, "tleP15_stability.npz"), qe=qe, qu=qu, b0=b0)
...
if __name__ == "__main__":
    print("=== TLE PRIMARY RE-RUN at U_TGT=0.15 ===", flush=True)
