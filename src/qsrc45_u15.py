#!/usr/bin/env python3
"""45-source (src45) core at U_TGT=0.15 (mirrors src/17 src45 core)."""
...
if __name__ == "__main__":
    ...
    print(f"  unservable classes at U=0.15 (max_m Q < 0.15): {len(unserv)} -> {unserv}", flush=True)
    rows = []
    ...
    with open(os.path.join(OUT, "src45_core_u15.csv"), "w", newline="") as f:
        w = csv.writer(f)
        w.writerow(["label", "down_mean", "minfill_mean", "servable_minfill",
                    "totQ_mean", "split_mean", "fill_q10", "fill_med"])
        w.writerows(rows)
    print("wrote data/qsrc/src45_core_u15.csv", flush=True)
